...
- [ ] Claude "원격 MCP 추가" 원클릭 로그인 검증 (사용자 인터랙티브 로그인 필요)
  ...
  - **2026-06-16 DCR confidential client 지원 추가**: 포트 이전 후 커넥터가 `POST /register`까지 도달했으나 400 거부. 원인 — Claude가 `token_endpoint_auth_method=client_secret_post`(confidential)로 DCR 요청하는데 onl1d DCR이 `none`(public)만 허용. 수정(onl1d `e40af23`): `register.ts`가 `client_secret_post`/`client_secret_basic` 등록 시 `client_secret` 발급(해시 저장)·응답 포함, 미지원 method는 400 유지, public 회귀 없음. 토큰 엔드포인트는 이미 confidential 인증 지원(변경 불필요). 테스트 70/70, 운영 재현 검증(201+client_secret). **남은 것: 사용자의 claude.ai 커넥터 재연결 테스트뿐.**
