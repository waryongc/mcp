...
- **nginx**: `/home/xiilab/svc/nginx/nginx.conf`의 `mcp.yeorot.cloud` 서버 블록. 디렉터리 마운트(`/etc/nginx-custom`)라 수정 후 `docker compose exec nginx nginx -s reload -c /etc/nginx-custom/nginx.conf` 로 반영됨
